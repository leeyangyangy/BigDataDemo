  ...
  <div class="mgmt-section">
    <div class="toolbar">
      <div class="search-bar">
        <input v-model="keyword" type="text" class="search-input" placeholder="搜索车间编码/名称..." @keyup.enter="loadData" />
      ...
      <button class="btn-create" @click="openCreate">+ 新建车间</button>
    </div>
      ...
      <table class="data-table" v-if="list.length > 0">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id">
            <td><strong>{{ item.workshopCode }}</strong></td>
    ...
    <div class="modal-overlay" v-if="showForm" @click.self="closeForm">
      <div class="modal-card">
        ...
        <div class="form-grid">
          <div class="form-field full" v-if="!isEdit">
            <label>车间编码 <span class="req">*</span></label>
            <input v-model="form.workshopCode" type="text" class="form-input" placeholder="如: WS001" />
          </div>
          <div class="form-field full">
            <label>车间名称 <span class="req">*</span></label>
            <input v-model="form.workshopName" type="text" class="form-input" placeholder="请输入车间名称" />
          </div>
          <div class="form-field">
            <label>类型</label>
            <select v-model="form.workshopType" class="form-input">
          ...
          <div class="form-field full">
            <label>描述</label>
            <textarea v-model="form.description" class="form-textarea" rows="2" placeholder="描述信息"></textarea>
          </div>
          <div class="form-field">
            <label>排序值</label>
...
async function loadData() {
  loading.value = true
  try {
    const res = await adminApi.workshop.getPage({
...
async function handleSubmit() {
  if (!form.value.workshopName?.trim()) { formMsg.value = '车间名称为必填项'; formMsgType.value = 'error'; return }
  if (!isEdit.value && !form.value.workshopCode?.trim()) { formMsg.value = '车间编码为必填项'; formMsgType.value = 'error'; return }
  ...
  try {
    let res
    if (isEdit.value) {
      res = await adminApi.workshop.update(editId.value, form.value)
    } else {
...
async function handleDelete(item) {
  if (!confirm(`确定删除车间 "${item.workshopName}" (${item.workshopCode}) 吗？`)) return
